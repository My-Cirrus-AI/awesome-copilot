...
Starting with **v1.0.71**, canvas support is also available directly in the **GitHub Copilot CLI** — you no longer need the desktop app to use extension-driven canvases. The CLI renders canvases in the terminal and supports the same extension capabilities (callable tools, state management, session-scoped or persistent storage) as the app.
