...
The `/worktree` command (v1.0.61+) creates a new git worktree on a fresh branch and switches into it, **leaving your uncommitted changes behind** in the original worktree. Use it when you want to start a clean parallel branch without carrying over in-progress work:
...
The **`/move`** command (v1.0.71+) is the complement to `/worktree`: it creates a new git worktree and **carries your uncommitted changes into it**. Use `/move` when you've started work on the wrong branch and want to continue on a new one without losing what you've done:

```
/move my-feature-branch
```

In summary: `/worktree` = new worktree, clean slate; `/move` = new worktree, bring your changes.
...
> **Plan mode enforcement (v1.0.71+)**: In plan mode, built-in tools that would modify the workspace are **hard-blocked** — the agent cannot edit files or run mutating shell commands while planning. Built-in mutators such as opening a pull request are also blocked. MCP and external tools are still allowed. This ensures plan-mode output is purely advisory until you approve and switch modes.
