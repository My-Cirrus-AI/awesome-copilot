...
As of v1.0.71, the **default maximum nesting depth was lowered from 6 to 4** to curb runaway recursive subagent delegation. Usage-based billing users can raise this limit up to 128 via the `subagents.maxDepth` setting.

**Can I send follow-up messages to a subagent that's still running?**

Yes, as of v1.0.72, **multi-turn subagents are always enabled**. You can send follow-up messages to a running subagent to provide clarification or redirect its work without waiting for it to finish its current task.
