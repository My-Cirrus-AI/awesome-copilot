...
> **Open Plugin Spec v1 compatibility (v1.0.74+)**: In addition to the `plugin.json` format above, Copilot CLI also accepts plugin manifests that follow the [Open Plugin Spec v1](https://openpluginspec.org/) format and `mcp.json` configuration files used by other AI tools. This means community plugins originally authored for other runtimes can be installed and used in Copilot CLI without modification.
